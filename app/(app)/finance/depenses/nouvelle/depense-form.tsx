...
export function DepenseForm() {
  ...
  // Pre-remplissage suite à un scan IA : on met à jour uniquement les
  // champs qui ont été extraits avec succès (montant + date + categorie
  // + description). Le sélecteur d'enveloppe se synchronise via
  // handleCategorieChange si l'utilisateur n'a pas déjà choisi
  // manuellement.
  const handleScanResult = (data: TicketPrefill) => {
    if (data.date) setDate(data.date);
    if (data.montant) setMontant(data.montant);
    if (data.categorie) handleCategorieChange(data.categorie);
  ...
  return (
    ...
    >
      ...
      {/* Bloc IA : scan ticket vision (V2-AI uniquement) */}
        ...
        <div className="flex items-start gap-3">
          <div className="flex-1">
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">
              ...
              automatiquement (montant, date, catégorie, description).
            </p>
          </div>
          <ScanTicketButton
